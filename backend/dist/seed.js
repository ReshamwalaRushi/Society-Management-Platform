...
async function seed() {
    await mongoose_1.default.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/society_management');
    console.log('Connected to MongoDB');
    // Clean existing data
    await Promise.all([
        User_1.default.deleteMany({}),
        Unit_1.default.deleteMany({}),
        Resident_1.default.deleteMany({}),
        Bill_1.default.deleteMany({}),
        Complaint_1.default.deleteMany({}),
        Announcement_1.default.deleteMany({}),
        Visitor_1.default.deleteMany({}),
        Vehicle_1.default.deleteMany({}),
        FacilityBooking_1.default.deleteMany({})
    ]);
    ...
    const residentUser1 = await User_1.default.create({
        name: 'Rahul Sharma',
    ...
    const residentUser2 = await User_1.default.create({
        name: 'Priya Singh',
    ...
    const residentUser3 = await User_1.default.create({
        name: 'Amit Kumar',
    ...
    const residents = await Resident_1.default.insertMany([
        {
    ...
    await Unit_1.default.findByIdAndUpdate(units[0]._id, { owner: residents[0]._id, currentOccupant: residents[0]._id });
    await Unit_1.default.findByIdAndUpdate(units[1]._id, { currentOccupant: residents[1]._id });
    await Unit_1.default.findByIdAndUpdate(units[2]._id, { owner: residents[2]._id, currentOccupant: residents[2]._id });
    // Create bills
    const now = new Date();
    const month = now.getMonth() + 1;
    const year = now.getFullYear();
    const dueDate = new Date(year, month, 10);
    await Bill_1.default.insertMany([
        {
    ...
    await Announcement_1.default.insertMany([
        {
    ...
    // Create visitors
    await Visitor_1.default.insertMany([
        {
    ...
    // Create vehicles
    await Vehicle_1.default.insertMany([
        {
            resident: residents[0]._id, unit: units[0]._id,
    ...
    await FacilityBooking_1.default.insertMany([
        {
